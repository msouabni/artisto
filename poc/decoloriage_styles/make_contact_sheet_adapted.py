"""S0-pivot-adapted - Planche contact 4 lignes (styles) x 3 colonnes (sujets).

Lit poc/decoloriage_styles/corpus_adapted.json (12 entrees), charge les PNG, et
compose une planche 3 colonnes (dog/castle/peacock) x 4 lignes
(low_poly/stained_glass/papercut/kawaii_bold), chaque vignette labellee, titre en tete.

Reutilise/adapte le code PIL de make_contact_sheet_s0.py.

Usage :
    python poc/decoloriage_styles/make_contact_sheet_adapted.py
"""
...
DEFAULT_CORPUS = Path(__file__).parent / "corpus_adapted.json"
DEFAULT_OUT = Path(__file__).parent / "contact_sheet_adapted.png"
...
LABEL_H = 62
HEADER_H = 90
ROW_LABEL_W = 150  # bandeau gauche avec le nom du style (plus large : noms longs)
BG = (245, 245, 247)
CARD_BG = (255, 255, 255)
TEXT = (30, 30, 35)
SUB = (110, 110, 120)
...
def build_contact_sheet(corpus_path: Path, out_path: Path) -> Path:
    ...
    # Header
    title = "S0-pivot-adapted - Decoloriage-styles - Styles coloriage-adaptes (12 PNG ERNIE)"
    subtitle = (
        f"Source : {data['source']}   |   Frozen : {data['frozen_at']}   |   "
        f"seed=42 fixe   |   styles adaptes par construction"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Planche contact S0-pivot-adapted (4x3)")
