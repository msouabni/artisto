"""POC vision QC prompts — variante gemma4:26b.

Clone fonctionnel de ``scripts/poc_vision_qc_prompts.py``. Seule différence :
modèle vision = ``gemma4:26b`` au lieu de ``qwen3.5:9b``. Mêmes 5 prompts
(A/B/C/D/E), mêmes 30 images de ``docs/reports/poc-batch-size/``, même
ground truth, mêmes métriques TP/FP/FN/TN/Precision/Recall/F1.

CLAUDE.md identifie gemma4:26b comme "arbitre vision" (plus précis sur les
couleurs subtiles, 6/6 vs 5/6 sur mini-bench gemma vs qwen). Ce POC mesure
si cette précision supérieure se traduit en détection effective du défaut
anatomy ``3_jambes`` que qwen3.5:9b a totalement raté (recall=0 sur 4/5
prompts en POC précédent).

Usage :
    python scripts/poc_vision_qc_gemma.py
"""
...
INDEX_JSON = OUTPUT_DIR / "poc-vision-qc-gemma.json"

VISION_MODEL = "gemma4:26b"   # ← seule différence vs poc_vision_qc_prompts.py
VISION_TIMEOUT = 180          # gemma4:26b ~19s/image — marge à 180s
...
def detect_defect(parsed: dict) -> tuple[bool, str]:
    """Retourne (detected, reason). Couvre tous les schémas des 5 prompts."""
...
def call_vision(prompt_id: str, image_path: Path) -> dict:
    """Appelle ``gemma4:26b`` avec le prompt {prompt_id} et l'image."""
    ...
    # gemma4:26b n'a pas le tag /no_think ni "think" natif (qwen3.5+ uniquement) —
    # _supports_native_think_disable retourne False pour gemma4, donc rien à passer.
...
def main() -> int:
    ...
    print(f"POC vision QC prompts — {VISION_MODEL} × 5 prompts × 30 images soccer", flush=True)
    print("=" * 100, flush=True)
    ...
    if missing_ann:
        print(f"⚠ {len(missing_ann)} images sans annotation, ignorées : {missing_ann[:3]}…", flush=True)
    ...
    print(f"Modèle vision : {VISION_MODEL} (timeout {VISION_TIMEOUT}s par appel)", flush=True)
    print(flush=True)
    ...
    for prompt_id in PROMPTS:
        ...
        for i, fname in enumerate(sorted_filenames, 1):
            ...
            print(
                f"  Prompt {prompt_id} — image {i:>2}/{len(sorted_filenames)} — "
                f"{entry['latency_s']}s  {tag:<10}  gt={gt_tag} det={det_tag} {mark} — {fname[-40:]}",
                flush=True,
            )
    ...
    print("=" * 100, flush=True)
    print(f"Tableau comparatif {VISION_MODEL} (vs ground truth 3_jambes)", flush=True)
    print("=" * 100, flush=True)
