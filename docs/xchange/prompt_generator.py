...
# ===================================================================
DEFAULT_TAXONOMY = "coloring_taxonomy_full.json"
DEFAULT_CARTOGRAPHY = "taxonomy_production_cartography.json"
DEFAULT_SEO = "coloring_taxonomy_seo.json"
...
class PromptGenerator:
    ...
    def build_workflow_json(self, leaf_id: str, output_path: Optional[str] = None) -> Dict:
        ...
        # Indices des nodes dans la liste
        # 0:UnetLoader 1:VAE 2:CLIP 3:Latent 4:KSampler 5:VAEDecode 6:Save 7:Neg 8:Pos
        
        # Latent : résolution
        wf['nodes'][3]['widgets_values'] = [result['resolution'][0], result['resolution'][1], 1]
        
        # Negative
        wf['nodes'][7]['widgets_values'] = [result['negative']]
        
        # Positive
        wf['nodes'][8]['widgets_values'] = [result['positive']]
        
        # Filename prefix
        wf['nodes'][6]['widgets_values'] = [f"alwan_{leaf_id}"]
    ...
    def batch(self, leaf_ids: List[str]) -> List[Dict]:
        """Génère N prompts en lot."""
        results = []
        for lid in leaf_ids:
            try:
                results.append(self.build_prompt(lid))
            except ValueError as e:
                results.append({'leaf_id': lid, 'error': str(e)})
        return results
    
    def list_subcategory_leaves(self, sub_id: str) -> List[str]:
        """Liste les leaf_ids d'une sous-catégorie."""
        for root, sub, leaf in self.leaf_index.values():
            if sub['id'] == sub_id:
                pass  # La structure est déjà traversée, on récupère via re-parcours
        return [
            leaf['id']
            for root in self.taxonomy
            for sub in root.get('children', [])
    ...
    def list_high_confidence_leaves(self) -> List[str]:
        """Liste toutes les feuilles dont la sous-cat est en confiance Haute."""
        out = []
        for root in self.taxonomy:
            for sub in root.get('children', []):
                strat = self.strategy_index.get(sub['id'])
                if strat and strat['confidence'].startswith('Haute') and 'PIL' not in strat['pipeline'] and 'SVG paramétrique' != strat['pipeline']:
                    out.extend(leaf['id'] for leaf in sub.get('children', []))
        return out
    
    def stats(self) -> Dict:
        """Statistiques globales."""
        total_leaves = len(self.leaf_index)
        high_conf = len(self.list_high_confidence_leaves())
        return {
            'total_leaves': total_leaves,
            'high_confidence_leaves': high_conf,
            'percentage_high_confidence': round(100 * high_conf / total_leaves, 1)
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Générateur de prompts pour Alwan Books",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('--leaf', type=str, help='ID de la feuille à générer')
    parser.add_argument('--subcategory', type=str, help='ID de la sous-catégorie (génère toutes les feuilles)')
    parser.add_argument('--batch', type=str, choices=['high_confidence', 'all'],
                        help='Lot prédéfini')
    parser.add_argument('--output', type=str, help='Fichier JSON de sortie pour le batch')
    parser.add_argument('--workflow', type=str, help='Fichier de sortie workflow ComfyUI (.json)')
    parser.add_argument('--taxonomy', type=str, default=DEFAULT_TAXONOMY)
    parser.add_argument('--cartography', type=str, default=DEFAULT_CARTOGRAPHY)
    parser.add_argument('--seo', type=str, default=DEFAULT_SEO)
    parser.add_argument('--stats', action='store_true', help='Affiche les stats globales')
    
    args = parser.parse_args()
    
    gen = PromptGenerator(args.taxonomy, args.cartography, args.seo)
    
    if args.stats:
        s = gen.stats()
        print(f"Total feuilles : {s['total_leaves']}")
        print(f"Feuilles haute confiance : {s['high_confidence_leaves']} ({s['percentage_high_confidence']}%)")
        return
    
    if args.leaf:
        if args.workflow:
            wf = gen.build_workflow_json(args.leaf, args.workflow)
            print(f"Workflow ComfyUI sauvegardé : {args.workflow}")
        else:
            result = gen.build_prompt(args.leaf)
            print("=" * 70)
            print(f"PROMPT POUR : {result['leaf_name_en']} ({result['leaf_id']})")
            print("=" * 70)
            print(f"Sous-catégorie : {result['subcategory_name']}")
            print(f"Classe : {result['workflow_class']}")
            print(f"Technique : {result['technique']}")
            print(f"Pipeline : {result['pipeline']}")
            print(f"Confiance : {result['confidence']}")
            print(f"Résolution : {result['resolution'][0]}x{result['resolution'][1]}")
            if result['pitfalls']:
                print(f"\nPièges connus :")
            ...
            print(f"\nNEGATIVE PROMPT:\n{result['negative']}")
            if result.get('seo'):
                print(f"\nSEO : volume={result['seo']['volume_bucket']}, "
                      f"priorité EN={result['seo']['priority_en']}")
        return
    
    if args.subcategory:
        leaf_ids = gen.list_subcategory_leaves(args.subcategory)
        if not leaf_ids:
            print(f"Sous-catégorie introuvable : {args.subcategory}", file=sys.stderr)
            sys.exit(1)
        results = gen.batch(leaf_ids)
        out = args.output or f"{args.subcategory}_prompts.json"
        with open(out, 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
        print(f"Généré {len(results)} prompts dans {out}")
        return
    
    if args.batch:
        if args.batch == 'high_confidence':
            leaf_ids = gen.list_high_confidence_leaves()
        else:
            leaf_ids = list(gen.leaf_index.keys())
        results = gen.batch(leaf_ids)
        out = args.output or f"batch_{args.batch}.json"
        with open(out, 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
        print(f"Généré {len(results)} prompts dans {out}")
        return
