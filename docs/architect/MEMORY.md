...
| 2026-05-12 | Annotation des 13 paires v1/v2 du re-run `2_objets` | à faire | Dans `poc-scale-benchmark` mais pas encore revues avec la grille v2 (annotations actuelles = héritage v1 migré). Valide la sortie du fix templates (NEGATIVE_V3 + `_ISOLATION` + `LEAF_OVERRIDES`). Référence : `2026-05-09_poc-rerun-2objets.md`. |
...
| 2026-05-16 | Annotation des méta-patterns prioritaires (Grille imagier, Imagier 3×3, Frise narrative, Multi-sujets) | à faire | Dans `poc-scale-benchmark` mais pas encore revues avec la grille v2. Critique car `color_ratio` aveugle à la composition — seule validation visuelle peut confirmer la structure. |
...
| 2026-05-12 | Annotation revue de `poc-scale-benchmark` avec grille v2 (222 entrées) | à faire | Contient les 13 paires v1/v2 du rerun `2_objets`, les méta-patterns prioritaires (Grille / Imagier 3×3 / Frise / Multi-sujets) et les 12 classes "OU". Les 220 entrées migrées en v2 sont **un héritage** v1, pas une revue avec la nouvelle grille — il faut repasser dessus. |
